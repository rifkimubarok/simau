...
function getSekolah() {
      var table =   $('#tblsekolah').DataTable({
          ...
          "ajax": {
              "url": getbasepath()+"uptd/Sekolah/listsekolah",
              "type": "POST"
          }
      });
}
...
function ubahData(id){
  $.ajax({
    url:getbasepath()+"uptd/Sekolah/getDialogubah/" + id,
    type: "GET",
    dataType: "JSON",
...
function uploadFile(){
  ...
  if(a == true){
    $('#progresstext').text(" ");
        ...
        var loop1 =0;
        $.ajax({
            url: getbasepath()+'uptd/Sekolah/getDataProfile/', // point to server-side controller method
...
function importData(){
  $.ajax({
    url:getbasepath()+"uptd/Sekolah/getDialogImport",
    type: "GET",
    success:function(data){
...
function simpanData() {
    ...
    $.ajax({
        type: "POST",
        url: getbasepath() + "uptd/Sekolah/simpanData",
