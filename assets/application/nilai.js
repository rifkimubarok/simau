...
function getnilai() {
  var table = $('#tbllist').DataTable({
    ...
    "ajax": {
      "url": getbasepath() + "dashboard/nilai/listnilai",
      "type": "POST",
      "data": function (d) {
        d.tahun = get_tahun();
      }
...
function get_tahun() {
  return $('#tahun_ajar').val();
}

function importData() {
  $.ajax({
    url: getbasepath() + "dashboard/nilai/getDialogImport",
    type: "GET",
...
function getnilaiTemp() {
  var table = $('#tblImport').DataTable({
    ...
    // Load data for the table's content from an Ajax source
    "ajax": {
      "url": getbasepath() + "dashboard/nilai/listnilaiTemp",
      "type": "POST"
    }
  });
}
...
function uploadFile() {
  ...
  if (a == true) {
    ...
    $.ajax({
      url: getbasepath() + 'dashboard/nilai/uploadDatanilai/', // point to server-side controller method
...
function saveDataFix() {
  $.ajax({
    url: getbasepath() + "dashboard/nilai/saveData",
    type: "POST",
    dataType: "json",
    data: {
      tahun: $('#tahun').val()
