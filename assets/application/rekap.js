...
function getrekap() {
      var table =   $('#tbllist').DataTable({
          ...
          "ajax": {
              "url": getbasepath()+"pengelola/nilai/rekap/listrekap",
              "type": "POST"
          }
      });
}
...
function importData(){
  $.ajax({
    url:getbasepath()+"pengelola/nilai/rekap/getDialogImport",
    type: "GET",
...
function getrekapTemp() {
      var table =   $('#tblImport').DataTable({
          ...
          // Load data for the table's content from an Ajax source
          "ajax": {
              "url": getbasepath()+"pengelola/nilai/rekap/listrekapTemp",
              "type": "POST"
          }
      });
}
...
function uploadFile(){
  ...
  if(a == true){
    ...
    var bar = document.getElementById("progressStatus");
        ...
        $.ajax({
            url: getbasepath()+'pengelola/nilai/rekap/uploadDatarekap/', // point to server-side controller method
...
function saveDataFix(){
  $.ajax({
      url: getbasepath()+"pengelola/nilai/rekap/saveData",
      type: "POST",
      dataType: "json",
      success:function(data){
