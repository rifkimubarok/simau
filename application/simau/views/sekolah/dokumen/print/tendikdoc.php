...
class PDF extends FPDF{
  function Header(){
   ...
   $this->cell(2,1,'Dokumen Laporan Tendik Pensiun',0,'L','L');
   $this->Ln();
   $this->SetFont('Arial','B','9');
   $this->Cell(1,1,'No','TB',0,'C',1); // cell dengan panjang 1
   $this->Cell(4,1,'NIP/NUPTK','TB',0,'L',1); // cell dengan panjang 1
   $this->Cell(5.2,1,'Nama Lengkap','TB',0,'L',1); // cell dengan panjang 2
   $this->Cell(4,1,'Tmp, Tgl Lahir','TB',0,'L',1); // cell dengan panjang 3
   $this->Cell(3.9,1,'Pangkat','TB',0,'L',1); // cell dengan panjang 3
   $this->Cell(4.9,1,'Asal Sekolah','TB',0,'L',1); // cell dengan panjang 3
   $this->Cell(4,1,'Jenis PTK','TB',0,'L',1); // cell dengan panjang 3
 ...
 foreach ($hasil as $cell) {
  ...
  if($no%13 == 0){
   ...
   if($cell->nip != null){
   	$pdf->Cell(4,1,$cell->nip,'B',0,'L'); // cell dengan panjang 1
   }else{
   	$pdf->Cell(4,1,$cell->nuptk,'B',0,'L'); // cell dengan panjang 1
   }
   $pdf->Cell(5.2,1,$cell->nama,'B',0,'L'); // cell dengan panjang 2
   $pdf->Cell(4,1,$cell->ttl,'B',0,'L'); // cell dengan panjang 3
   $pdf->Cell(3.9,1,$cell->pangkat_gol,'B',0,'L'); // cell dengan panjang 3
   $pdf->Cell(4.9,1,$lib->getNameSekolah($cell->kode_sekolah),'B',0,'L');
   $pdf->Cell(4,1,$cell->jenis_ptk,'B',0,'L');
  $pdf->Ln();
  $break;
   $newLine;
 }

 $pdf->Output('laporanTendikPensiun'.$this->session->userdata('npsn').'.pdf','D');
?>