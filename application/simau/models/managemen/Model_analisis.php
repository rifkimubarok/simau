...
class Model_analisis extends CI_Model
{
    ...
    public function getGrafNilai($kec,$npsn)
    {
    	...
    	if($kec != '' and $npsn == ''){
    		...
    		$this->db->select('kabupaten,kd_matpel,(select a.matpel from ref_matpel a where  a.id = kd_matpel) as matpel,ifnull(round(avg(ratarata),2),0) as nilai');
    		$this->db->group_by('1,2');
            $this->db->where('jenjang',$jenjang);
            $this->db->where("thn_upload",$this->input->post('thn_upload'));
    		$hasil = $this->db->get('v_rekapjwbkec');
    		if($hasil->num_rows()>0){
    			return $hasil->result();
    		}else{
    			return null;
    ...
    public function getGrafNilaiv($kode,$matpel,$param)
    {
        ...
        $this->db->select('kabupaten,kecamatan,'.$mata.',jenjang,(indo+ipa+matematika) as nilai');
