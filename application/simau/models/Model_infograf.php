...
class Model_infograf extends CI_Model
{
    ...
    function getBanyakTotalGuru(){
        if($this->session->userdata('level') == '7' || $this->session->userdata('npsn') == "8"){
            $this->db->where("tbl_sekolah.jenjang",$this->session->userdata('npsn'));
        }
        if($this->session->userdata('level')=="9"){
            $this->db->where("tbl_sekolah.kecamatan",$this->session->userdata('kecamatan'));
        }
