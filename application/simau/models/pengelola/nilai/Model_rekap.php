...
{
	private $table = 'tmp_rkp_kelulusan';
    public $_table = 'tbl_rkp_kelulusan';
    ...
    public function getsinglerow($param){
        $a = $this->db->query('SELECT * FROM tbl_rkp_kelulusan WHERE id = "'.$param.'"');
        return $a->row();
    }
    ...
    public function simpanDataFix($id_uploader){
        ...
        if($delOld > 0){
            $hasil = $this->db->query('insert into '.$this->_table.' select "" as id,tahun,jml_peserta,lulus,tidak_lulus,jml,id_uploader from '.$this->table.' where id_uploader = "'.$id_uploader.'"');
            if($hasil > 0){
    ...
    var $column_search1 = array('id',
        ...
        'tidak_lulus',
        'jml'); 
    var $order1 = array('tahun' => 'asc'); // default order
