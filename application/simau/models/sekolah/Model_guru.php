...
class Model_guru extends CI_Model
{
    ...
    public function getsinglerow($param){
        $a = $this->db->query('SELECT * FROM tbl_guru WHERE id = "'.$param.'"');
        return $a->row();
    }
    ...
    private function _get_datatables_query2()
    {
        ...
        $this->db->join("v_umurguru b",'on a.id = b.id ','inner');
    ...
    {
        $where = array("a.kode_sekolah"=>$this->session->userdata('npsn'),"b.umur"=>"60");
        $this->db->where($where);
    }
    ...
    public function getAllDataGuru($where)
    {
           $this->db->select('a.id,a.nip,a.nama,concat(a.tmp_lahir," ,",a.tgl_lahir) as ttl,a.pangkat_gol,a.kode_sekolah,a.jenis_ptk,a.nuptk');
        $this->db->join("v_umurguru b",'on a.id = b.id ','inner');
        $this->db->from($this->_table.' a');
