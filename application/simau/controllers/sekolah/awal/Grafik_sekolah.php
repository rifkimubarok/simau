...
class Grafik_sekolah extends CI_Controller {
    ...
    public function getInformasi()
    {
        ...
        if($data->num_rows() >0){
            $hasil = $data->row();
            $data = array("totpeg"=>number_format($hasil->totpeg,0),"totmula"=>number_format($hasil->totmula,0),"totmupe"=>number_format($hasil->totmupe,0),"totmu"=>number_format($hasil->totmu,0));
        echo json_encode($data);
        }else{
