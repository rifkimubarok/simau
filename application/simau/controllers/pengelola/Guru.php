...
class Guru extends CI_Controller
{
	...
	{
		parent::__construct();
		if ($this->session->userdata('status')==false or $this->session->userdata('level') != '2') {
            redirect('Landing');
        }
		$this->load->library('Template');
		$this->load->library(array('PHPExcel','PHPExcel/IOFactory'));
		$this->load->model('pengelola/Model_guru','guru');
		$this->load->library('Libkebutuhan');
	}

	 function index(){
		$this->template->display("pengelola/guru/listguru");
	}

    function info(){
        $this->template->display("pengelola/grafik/grafikUmurGuru");
    }

    function infoDoc(){
        $this->template->display("pengelola/dokumen/listguru");
    }

	function infoGrafPangkat(){
		$this->template->display("pengelola/grafik/grafikPangkat");
	}

    function infoBerkala(){
        $this->template->display("pengelola/dokumen/listguruberkala");
    }

	function infoPangkat(){
		$this->template->display("pengelola/dokumen/listgurupangkat");
	}
	...
	function cetak_dokumen($kode_sekolah=null)
	{
		if($kode_sekolah!= null){
			...
			/*$this->load->view('pengelola/dokumen/print/gurudoc',$data);*/
