...
class Sekolah extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('uptd/model_profile','profile');
		$this->load->model('uptd/model_sekolah','sekolah');
	}

	 function index(){
		$this->template->display("uptd/sekolah/index");
	}
	...
	{
		$this->load->view('uptd/profile/profile');
	}
	function listsekolah(){
        ...
        foreach ($list as $sekolah) {
            ...
            $row[] = '<div class="btn-group">
	                  <button class="btn btn-xs btn-outline btn-primary" onclick="ubahData('.$id.')" style="display: block;"><i class="fa fa-eye"></i></button>
	                  </div>';
            $data[] = $row;
        }
	...
	public function fixubah(){
		if($this->input->post('jenjang') == 'SMK'){
			$icon = 'university2.png';
		}else if($this->input->post('jenjang') == 'SMA'){
			$icon = 'university1.png';
		}else if($this->input->post('jenjang') == 'SLB'){
