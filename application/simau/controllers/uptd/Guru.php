...
class Guru extends CI_Controller
{
	...
	{
		parent::__construct();
		if ($this->session->userdata('status')==false or $this->session->userdata('level') != '9') {
            redirect('Landing');
        }
		$this->load->library('Template');
		$this->load->library(array('PHPExcel','PHPExcel/IOFactory'));
		$this->load->model('uptd/Model_guru','guru');
		$this->load->library('Libkebutuhan');
	}

	 function index(){
		$this->template->display("uptd/guru/listguru");
	}

    function info(){
        $this->template->display("uptd/grafik/grafikUmurGuru");
    }

    function infoDoc(){
        $this->template->display("uptd/dokumen/listguru");
    }

	function infoGrafPangkat(){
		$this->template->display("uptd/grafik/grafikPangkat");
	}

    function infoBerkala(){
        $this->template->display("uptd/dokumen/listguruberkala");
    }

	function infoPangkat(){
		$this->template->display("uptd/dokumen/listgurupangkat");
	}
	...
	function cetak_dokumen($kode_sekolah=null)
	{
		if($kode_sekolah!= null){
			...
			/*$this->load->view('uptd/dokumen/print/gurudoc',$data);*/
