...
function gen_menu1() {
	...
	if ($sess_level == "guru") {
	  $menu = array(
	            array("icon"=>"dashboard", "url"=>"adm/", "text"=>"Dashboard","jenis"=>"normal"),
	            array("icon"=>"list-alt", "url"=>"adm/m_soal", "text"=>"Soal","jenis"=>"normal"),
	            array("icon"=>"file", "url"=>"adm/m_ujian", "text"=>"Ujian","jenis"=>"normal"),
	            array("icon"=>"file", "url"=>"adm/h_ujian", "text"=>"Hasil UKG","jenis"=>"normal"),
	          );
	} else if ($sess_level == "siswa") {
	  $menu = array(
	            array("icon"=>"dashboard", "url"=>"adm/", "text"=>"Dashboard","jenis"=>"normal"),
	            array("icon"=>"file", "url"=>"adm/ikuti_ujian", "text"=>"Ujian","jenis"=>"normal"),
	          );
	} else if ($sess_level == "admin") {
	  $menu = array(
	            array("icon"=>"dashboard", "url"=>"adm/", "text"=>"Dashboard","jenis"=>"normal"),
	            array("icon"=>"list-alt", "url"=>"#", "text"=>"Data","jenis"=>"dropdown","data"=>array(
		            array("icon"=>"list-alt", "url"=>"adm/m_guru", "text"=>"Data Guru","jenis"=>"normal"),
		            array("icon"=>"list-alt", "url"=>"adm/m_user", "text"=>"Data User","jenis"=>"normal"),
		            array("icon"=>"list-alt", "url"=>"adm/m_mapel", "text"=>"Data Mapel","jenis"=>"normal"),
		            array("icon"=>"list-alt", "url"=>"modul/m_modul", "text"=>"Data Modul","jenis"=>"normal"))),
	            array("icon"=>"list-alt", "url"=>"adm/m_soal", "text"=>"Soal","jenis"=>"normal"),
	            array("icon"=>"file", "url"=>"adm/m_ujian", "text"=>"Ujian","jenis"=>"normal"),
	            array("icon"=>"file", "url"=>"adm/h_ujian", "text"=>"Hasil UKG","jenis"=>"normal"),
	            array("icon"=>"bar-chart-o", "url"=>"#", "text"=>"Grafik","jenis"=>"dropdown","data"=>array(
	            	array("icon"=>"bar-chart-o", "url"=>"adm/grafik_hasil", "text"=>"Grafik Hasil UKG (Total)","jenis"=>"dropdown"),
	            	array("icon"=>"bar-chart-o", "url"=>"adm/grafik_hasil_persen", "text"=>"Grafik Hasil UKG (%)","jenis"=>"dropdown"),
	            	array("icon"=>"bar-chart-o", "url"=>"adm/grafik_hasil_persen_modul", "text"=>"Grafik Hasil UKG (Modul)","jenis"=>"dropdown")
	            ))
	          );
	} else {
	  $menu = array(
	...
	if ($menu != null) {
		    ...
		    foreach ($menu as $m) {
		        ...
		        if($m['jenis'] == "normal"){
		        	if ($url == $url1[1]) {
			          echo ' <li><a href="'.base_url().$m['url'].'"><i class="fa fa-'.$m['icon'].' nav_icon"></i> '.$m['text'].' </a></li>';
			        } else {
			          echo ' <li><a href="'.base_url().$m['url'].'"><i class="fa fa-'.$m['icon'].' "></i> '.$m['text'].' </a></li>';
			        }
		        }else{
		        	$output = '<li>
                            <a href="#"><i class="fa fa-'.$m['icon'].'"></i> '.$m['text'].'<span class="fa arrow"></span></a>';
                    $output .= '<ul class="nav nav-second-level">';
                    	foreach ($m['data'] as $m2) {
		    				$url1 = explode('/', $m2['url']);
                    		if($url == $url1[1]){
                    			$output .= '<li><a href="'.base_url().$m2['url'].'"><i class="fa fa-'.$m2['icon'].' nav_icon"></i> '.$m2['text'].'</a></li>';
                    		}else{
                    			$output .= '<li><a href="'.base_url().$m2['url'].'"><i class="fa fa-'.$m2['icon'].'"></i> '.$m2['text'].'</a></li>';
                    		}
                    	}
                    $output .= "</ul></li>";
                    echo $output;
