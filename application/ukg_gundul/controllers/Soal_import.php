...
class Soal_import extends CI_Controller {
    ...
    public function soal() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            for ($j = $idx_baris_mulai; $j <= $idx_baris_selesai; $j++) {
                ...
                if ($soal != "") {
                    $data[] = "('".$p['id_guru']."','".$p['jenjang']."','".$p['modulpelajaran']."', '".$p['id_mapel']."', '".$bobot."', '".$soal."', '#####".$opsi_a."', '#####".$opsi_b."', '#####".$opsi_c."', '#####".$opsi_d."', '#####".$opsi_e."', '".$kunci."', NOW(), 0, 0)"; 
                }
            }

            $strq = "INSERT INTO m_soal (id_guru,jenjang,id_modul, id_mapel, bobot, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, jawaban, tgl_input, jml_benar, jml_salah) VALUES ";
