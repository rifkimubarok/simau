...
class Adm extends CI_Controller {
	...
	public function m_guru() {
		...
		if ($uri3 == "det") {
			...
			$this->load->model("Model_matapelajaran",'matpel');
	        ...
	        foreach ($q_datanya as $d) {
	            ...
	            $data_ok[] = $d['nama'];
	            $data_ok[] = $d['nim'];
	            $data_ok[] = $this->modul->getMatpelParam($d['jenjang'],$d['id_matpel']);
	...
	public function m_soal() {
		...
		if ($uri3 == "det") {
			...
			if (!empty($data)) {
		        ...
		        $query_where = '';
		        if($id_mapel != ''){
		        	$query_where .= 'and a.id_mapel = "'.$id_mapel.'"';
		        }
		        if($id_modul != ''){
		        	$query_where .= 'and a.id_modul = "'.$id_modul.'"';
		        }
		        ...
		        $d_total_row = $this->db->query("SELECT a.*
												...
												OR c.nama LIKE '%".$search['value']."%')".$query_where)->num_rows();
		        ...
		        $q_datanya = $this->db->query("SELECT a.*, b.nama nmguru, c.nama nmmapel
		                                        ...
		                                        WHERE ".$wh." (a.soal LIKE '%".$search['value']."%' 
												OR b.nama LIKE '%".$search['value']."%' 
												OR c.nama LIKE '%".$search['value']."%') ".$query_where."
		        ...
		        foreach ($q_datanya as $d) {
		            $data_ok = array();
		            $persent = '*';
		            if($d['jml_benar']!= 0 || $d['jml_salah'] != 0){
		            	$persent = number_format(($d['jml_benar'] / ($d['jml_benar']+$d['jml_salah'])) * 100 );
		            }
		            $data_ok[0] = $no++;
		            $data_ok[1] = substr($d['soal'], 0, 300);
		            $data_ok[2] = $d['nmmapel'].'<br>'.$d['nmguru'];
		            $data_ok[3] = "Jml dipakai : ".($d['jml_benar']+$d['jml_salah'])."<br>Benar: ".$d['jml_benar'].", Salah: ".$d['jml_salah']."<br>Persentase benar : ".$persent." %";
		            $data_ok[4] = '<div class="btn-group">
	                          <a href="'.base_url().'adm/m_soal/edit/'.$d['id'].'" class="btn btn-info btn-xs"><i class="fa fa-pencil" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Edit</a>
	                          <a href="'.base_url().'adm/m_soal/hapus/'.$d['id'].'" class="btn btn-danger btn-xs"><i class="fa fa-remove" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Hapus</a>
	                         ';
	...
	public function m_ujian() {
		...
		if ($uri3 == "det") {
			...
			if (!empty($a)) {
				...
				$are['id_modul'] = $a->id_modul;
				$are['jenjang'] = $a->jenjang;
			...
			if ($p->id != 0) {
				...
				$this->db->query("INSERT INTO tr_guru_tes VALUES (null, '".$a['sess_konid']."', '".bersih($p,"mapel")."', '".$p->modulpelajaran."','".$p->jenjang."',
			...
			$ambil_data = $this->db->query("SELECT id FROM m_soal WHERE id_mapel = '$id_mapel' and id_modul= '$id_modul' AND id_guru = '".$a['sess_konid']."'")->num_rows();
			...
			exit();			
		        ...
		        foreach ($q_datanya as $d) {
		            ...
		            $data_ok[2] = $d['mapel'].' '.$this->modul->getNamaModul($d['id_mapel'],$d['id_modul']);
	...
	public function ikuti_ujian() {
		...
		$jeson = array();
		//$a['sess_konid']
		$jenjang = $this->modul->getJenjang($this->session->userdata('admin_user'));
		$a['data'] = $this->db->query("SELECT 
									...
									LEFT JOIN m_modul e on a.id_mapel = e.id_mapel and a.id_modul = e.id_modul
									where a.jenjang = '".$jenjang[0]."' and a.id_mapel = '".$jenjang[1]."'
	...
	public function ikut_ujian() {
		...
		if ($uri3 == "simpan_satu") {
			...
			*/
			...
			$a['du'] = $this->db->query("SELECT a.id, a.tgl_mulai, a.terlambat, d.modul, 
										a.token, a.nama_ujian, a.jumlah_soal, a.waktu,
										b.nama nmguru, c.nama nmmapel FROM tr_guru_tes a 
										INNER JOIN m_guru b ON a.id_guru = b.id
										INNER JOIN m_mapel c ON a.id_mapel = c.id 
										LEFT JOIN m_modul d on a.id_mapel = d.id_mapel and a.id_modul = d.id_modul
			...
			if ($cek_sdh_selesai < 1) {
				...
				$jml_total_soal = $this->db->query("select Count(id) AS jml_soal_total from m_soal where id_mapel = '".$cek_detil_tes->id_mapel."' AND id_modul = '".$cek_detil_tes->id_modul."'")->row();
				...
				if ($cek_sdh_ujian < 1)	{		
					$soal_urut_ok = array();
					$q_soal			= $this->db->query("SELECT id, file, tipe_file, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, '' AS jawaban FROM m_soal WHERE id_mapel = '".$cek_detil_tes->id_mapel."' AND id_guru = '".$cek_detil_tes->id_guru."' ".$acakan." LIMIT ".$jml_total_soal->jml_soal_total)->result();
					$i = 0;
					foreach ($q_soal as $s) {
						...
						if($i == (intval($cek_detil_tes->jumlah_soal)-1)){
							break;
						}else{
							$i++;
						}
				...
				if (!empty($soal_urut_ok)) {
				    foreach ($soal_urut_ok as $d) { 
				        ...
				        $html .= '<input type="hidden" name="id_soal_'.$no.'" id="id_soal_'.$no.'" value="'.$d->id.'">';
				        ...
				        for ($j = 0; $j < $this->config->item('jml_opsi'); $j++) {
				            ...
				            $value_key = 0;
				            if(strtoupper($this->opsi[$j]) == 'A'){
				            	$value_key =65;
				            }else if(strtoupper($this->opsi[$j]) == 'B'){
				            	$value_key =66;
				            }else if(strtoupper($this->opsi[$j]) == 'C'){
				            	$value_key =67;
				            }else if(strtoupper($this->opsi[$j]) == 'D'){
				            	$value_key =68;
				            }else if(strtoupper($this->opsi[$j]) == 'D'){
				            	$value_key =69;
				            }
	...
	public function login_admin() {
		$this->load->view('aaa_login_admin');
	}
            ...
            if(isset($result->token)){
                if(isset($result->data)){
                    ...
                    if($level == "siswa"){
                        ...
                        $data = array("username"=>$_data->username,"password"=>md5($_data->pengguna_id),"level"=>$level,"kon_id"=>$kon_id);
                        $where = array("username"=>$_data->username);
                        $id = $this->insert_data("m_admin",$data,$where);

                        $data = array(
                            'admin_id' => $id,
                            'admin_user' => $_data->username,
                            'admin_level' => $level,
                            'admin_konid' => $kon_id,
	...
	public function grafik_hasil()
	{
		$this->cek_aktif();
		$a['sess_level'] = $this->session->userdata('admin_level');
		$a['sess_user'] = $this->session->userdata('admin_user');
		$a['sess_konid'] = $this->session->userdata('admin_konid');
		$a['p']			= "v_graf_hasil_ukg";
		$this->load->view('aaa_mod', $a);
	}

	public function grafik_hasil_persen()
	{
		$this->cek_aktif();
		$a['sess_level'] = $this->session->userdata('admin_level');
		$a['sess_user'] = $this->session->userdata('admin_user');
		$a['sess_konid'] = $this->session->userdata('admin_konid');
		$a['p']			= "v_graf_hasil_ukg_persen";
		$this->load->view('aaa_mod', $a);
	}

	public function grafik_hasil_persen_modul()
	{
		$this->cek_aktif();
		$a['sess_level'] = $this->session->userdata('admin_level');
		$a['sess_user'] = $this->session->userdata('admin_user');
		$a['sess_konid'] = $this->session->userdata('admin_konid');
		$a['jenjang'] = $this->home->getJenjang();
		$a['mapel'] = $this->home->getMapel();
		$a['p']			= "v_graf_hasil_ukg_persen_permodul";
		$this->load->view('aaa_mod', $a);
	}
}