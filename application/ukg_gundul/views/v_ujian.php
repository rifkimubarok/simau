...
<nav class="navbar navbar-findcond navbar-fixed-top">
    <div class="container">
        <div class="navbar-header text-primary">
            ...
            <a class="navbar-brand text-primary"><i class="glyphicon glyphicon-user text-primary"></i> <?php echo $this->session->userdata('admin_nama'); ?></a>
        </div>

        <div class="collapse navbar-collapse" id="navbar">
            <ul class="nav navbar-nav navbar-right" style="z-index: 1000">
                <li><a href="<?php echo base_url(); ?>adm/logout" onclick="return confirm('keluar..?');"><i class="glyphicon glyphicon-share text-primary"></i> Keluar</a></li>
            </ul>
        </div>
    </div>
</nav>
...
<div class="modal fade" id="m_konfirmasi" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
            <p>
               Ujian Akan Diakhiri. Anda Yakin Ingin Mengakhiri Ujian ?
            </p>
      </div>
      <div class="modal-footer">
        <button class="btn btn-danger" onclick="return akhiri_ujian();"><i class="fa fa-check"></i> Akhiri</button>
...
<script type="text/javascript">
    ...
    $(document).on("ready", function(){
        ...
        $(this).on('keydown', function(event) {
            ...
            if (event.keyCode == 65) {
                set_jawaban_keyboard("A");
             }
            if (event.keyCode == 66) {
                set_jawaban_keyboard("B");
             }
            if (event.keyCode == 67) {
                set_jawaban_keyboard("C");
             }
            if (event.keyCode == 68) {
                set_jawaban_keyboard("D");
             }
            if (event.keyCode == 69) {
                set_jawaban_keyboard("E");
             }
            if (event.keyCode == 37) {
                back();
             }
    ...
    /*function test() {
        var a = $('#_form');
        console.log(a);
        var no_sekarang = $('#soalke').text();
        var b = $("#id_soal_"+no_sekarang).val();
        $("#opsi_B_"+b).attr("checked",'');
        $('#btn_soal_'+no_sekarang).attr("class",'btn btn-success btn_soal btn-sm');
        $('#btn_soal_'+no_sekarang).text(no_sekarang+'. B');
        console.log(no_sekarang+"-------"+b);    
    }*/

    function addChecked() {
        //$("#opsi_B_121").attr("checked",'');
    }

    function set_jawaban_keyboard(jawaban) {
        var no_sekarang = $('#soalke').text();
        var b = $("#id_soal_"+no_sekarang).val();
        //clear(jawaban,b);
        $("#opsi_"+jawaban+"_"+b).prop('checked', true);
        $('#btn_soal_'+no_sekarang).attr("class",'btn btn-success btn_soal btn-sm');
        $('#btn_soal_'+no_sekarang).text(no_sekarang+'. '+jawaban);
    }
