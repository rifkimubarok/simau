...
class Model_nilai extends CI_Model
{
    ...
    function _get_custom_field()
    {
        if($this->userdata->level != 1){
            $this->db->where("id_uploader",$this->userdata->id);
        }
        if($this->input->post("tahun") != ''){
            $this->db->where(array("tahun"=>$this->input->post("tahun")));
        }
    }
    ...
    public function simpanDataFix($id_uploader,$tahun){
        $delOld = $this->deleteDataOld($id_uploader,$tahun);
        if($delOld > 0){
            $hasil = $this->db->query("insert into $this->_table select '' as id,kode_sekolah,nama_sekolah,status_sekolah,peserta,tl,persen,bin,ing,mat,ipa,tot,rank,id_uploader,tahun,created_at,updated_at from $this->table where id_uploader = $id_uploader and tahun = '$tahun'");
            if($hasil > 0){
    ...
    function deleteDataOld($id_uploader,$tahun){
        $this->db->where(array("id_uploader"=>$id_uploader,"tahun"=>$tahun));
