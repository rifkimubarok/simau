...
if ( ! function_exists('substring_text')){
	function substring_text($input, $length, $ellipses = true, $strip_html = true) {
...
if(!function_exists('diff_tgl')){
	function diff_tg($tgl_mulai,$tgl_selesai){
		$tgl_fix = '';
		if((date('m',$tgl_mulai) == date('m',$tgl_selesai)) && (date('Y',$tgl_mulai) == date('Y',$tgl_selesai))){
			if(date('d',$tgl_mulai) == date('d',$tgl_selesai)){
				$tgl_fix =date('d F, Y',$tgl_mulai);
			}else{
				$tgl_fix = date('d',$tgl_mulai).date(' - d F, Y',$tgl_selesai);
			}
		}else if((date('m',$tgl_mulai) != date('m',$tgl_selesai)) && (date('Y',$tgl_mulai) == date('Y',$tgl_selesai))){
			$tgl_fix = date('d F',$tgl_mulai).date(' - d F, Y',$tgl_selesai);
		}else{
			$tgl_fix = date('d F, Y',$tgl_mulai).date(' - d F, Y',$tgl_selesai);
		}
		return $tgl_fix;
	}
}